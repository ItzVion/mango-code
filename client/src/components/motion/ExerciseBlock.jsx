...
export function ExerciseBlock({ id, prompt, starterCode }) {
  ...
  return (
    <div className="rounded-2xl border border-ink/10 bg-card p-5">
      ...
      <textarea value={code} onChange={(e) => setCode(e.target.value)} spellCheck={false} className="mt-3 h-40 w-full rounded-xl border border-ink/10 bg-ink p-4 font-mono text-sm text-paper outline-none focus:border-mango" />
      ...
      <AnimatePresence>
        {result && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className={`mt-3 flex items-start gap-2 rounded-xl border p-3 text-sm ${result.pass ? 'border-leaf/30 bg-leaf/10 text-leaf' : 'border-rose/30 bg-rose/10 text-rose'}`}>
