...
export function AnimationLayer() {
  ...
  return (
    <>
      <motion.div className="pointer-events-none fixed inset-0 z-[55] opacity-70" style={{ background: 'radial-gradient(600px circle at var(--mx, 50%) var(--my, 50%), rgba(245,166,35,.09), transparent 65%)' }} />
      ...
      <AnimatePresence>
        {bursts.map(b => <motion.div key={b.id} className="pointer-events-none fixed z-[75] h-0 w-0" style={{ left: b.x, top: b.y }} initial={{ opacity: 1 }} animate={{ opacity: 0 }} exit={{ opacity: 0 }} transition={{ duration: .8 }}>
          ...
          {Array.from({ length: BURST_PARTICLES }, (_, i) => {
            const angle = (Math.PI * 2 * i) / BURST_PARTICLES
            const distance = 42 + (i % 3) * 17
            return <motion.span key={i} className="absolute -left-1.5 -top-1.5 h-3 w-3 rounded-full bg-mango shadow-[0_0_12px_rgba(245,166,35,.75)]" initial={{ x: 0, y: 0, scale: 1, opacity: 1 }} animate={{ x: Math.cos(angle) * distance, y: Math.sin(angle) * distance, scale: 0, opacity: 0 }} transition={{ duration: .55 + (i % 4) * .05, ease: 'easeOut' }} />
          })}
        </motion.div>)}
      </AnimatePresence>
