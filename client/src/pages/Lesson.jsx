...
function LessonContent({ sections, step, hasExercise }) {
  ...
  const exampleCode = exampleParts[0] || sections.EXAMPLE || ''
  const exampleNote = exampleParts.slice(1).join('\n\n')
      ...
      <motion.section key={key} initial={{ opacity: 0, x: 24, scale: 0.985 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: -24, scale: 0.985 }} transition={{ duration: 0.25, ease: 'easeOut' }}>
        ...
        {key === 'EXAMPLE' && sections[key] ? (
          <div className="space-y-3">
            <motion.pre initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="overflow-x-auto rounded-2xl border border-black/10 bg-ink p-5 font-mono text-[13px] leading-6 text-paper shadow-sm"><code>{exampleCode}</code></motion.pre>
            {exampleNote && <p className="rounded-2xl border border-ink/10 bg-card p-4 text-sm leading-6 text-ink-soft">{exampleNote}</p>}
          </div>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-3xl pt-8 pb-16">
      ...
      <div className="mt-4 flex items-end justify-between gap-4">
        <div><TextEffect as="h2" className="text-2xl font-semibold" delay={0.08}>{lesson.title}</TextEffect>{!isTest && <p className="mt-2 text-sm text-ink-soft">Learn it → see it → try it → check it.</p>}</div>
      ...
      {isTest ? (
        ...
        <div className="mt-7">
          <div className="mb-5 flex gap-1.5" aria-label="Lesson progress">{steps.map((item, index) => <motion.div key={item.key} animate={{ width: index === step ? 38 : 10, opacity: index <= step ? 1 : 0.25 }} className="h-1.5 rounded-full bg-mango" />)}</div>
          <Buddy text={stepMeta.find((x) => x.key === currentKey)?.buddy || 'Nice work!'} step={step} />

          {isCheck ? (
            <AnimatePresence mode="wait"><motion.div key={currentQuestion?.id || 'check'} initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }}>{currentQuestion ? <QuizBlock id={currentQuestion.id} question={currentQuestion.question} options={currentQuestion.options} onAnswered={(result) => setAnsweredQuestions((current) => ({ ...current, [currentQuestion.id]: Boolean(result) }))} /> : <div className="rounded-2xl border border-leaf/30 bg-leaf/10 p-6 text-center"><Trophy className="mx-auto text-leaf" /><p className="mt-2 font-semibold">You made it!</p></div>}</motion.div></AnimatePresence>
          ) : <LessonContent sections={sections} step={step} hasExercise={hasExercise} />}

          {currentKey === 'YOUR_TURN' && hasExercise && <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-5">{lesson.exercises.map((ex) => <ExerciseBlock key={ex.id} id={ex.id} prompt={ex.prompt} starterCode={ex.starter_code} />)}</motion.div>}
