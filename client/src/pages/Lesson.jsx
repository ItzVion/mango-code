...
function Buddy({ text, step }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="mb-5 flex items-center gap-3 rounded-2xl border border-mango/30 bg-mango/10 px-4 py-3">
      <motion.div key={step} initial={{ rotate: -12, scale: 0.7 }} animate={{ rotate: [0, -7, 7, 0], scale: 1 }} transition={{ duration: 0.55 }} className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-mango text-2xl shadow-sm">
        {faces[step] || '🥭'}
      </motion.div>
    ...
    <AnimatePresence mode="wait">
      <motion.section key={key} initial={{ opacity: 0, x: 24, scale: 0.985 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: -24, scale: 0.985 }} transition={{ duration: 0.25, ease: 'easeOut' }}>
        <div className="mb-4 flex items-center gap-3">
          ...
          <div><p className="text-xs font-semibold uppercase tracking-wider text-ink-soft">Step {step + 1}</p><h3 className="text-xl font-semibold">{meta.title}</h3></div>
        </div>
        ...
        {key === 'EXAMPLE' && sections[key] ? (
          ...
          <div className="rounded-2xl border border-ink/10 bg-card p-5 text-[15px] leading-7 text-ink-soft shadow-sm"><p className="whitespace-pre-line">{sections[key] || 'Take a moment to review what you just learned.'}</p></div>
        )}
      </motion.section>
    </AnimatePresence>
  )
...
export default function Lesson() {
  ...
  const steps = useMemo(() => isTest ? [] : stepMeta.filter((item) => item.key !== 'YOUR_TURN' || sections.YOUR_TURN || hasExercise), [isTest, sections.YOUR_TURN, hasExercise])
      ...
      {isTest ? (
        ...
        <div className="mt-7">
          <div className="mb-5 flex gap-1.5" aria-label="Lesson progress">
            {steps.map((item, index) => <motion.div key={item.key} animate={{ width: index === step ? 38 : 10, opacity: index <= step ? 1 : 0.25 }} className="h-1.5 rounded-full bg-mango" />)}
          </div>
          ...
          {isCheck ? (
            <AnimatePresence mode="wait">
              <motion.div key={currentQuestion?.id || 'check'} initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }}>
                {currentQuestion ? (
                  <QuizBlock id={currentQuestion.id} question={currentQuestion.question} options={currentQuestion.options} onAnswered={(result) => setAnsweredQuestions((current) => ({ ...current, [currentQuestion.id]: Boolean(result?.correct || result) }))} />
                ) : (
                  <div className="rounded-2xl border border-leaf/30 bg-leaf/10 p-6 text-center"><Trophy className="mx-auto text-leaf" /><p className="mt-2 font-semibold">You made it!</p></div>
                )}
              </motion.div>
            </AnimatePresence>
          ) : (
          ...
          {currentKey === 'YOUR_TURN' && hasExercise && <div className="mt-5">{lesson.exercises.map((ex) => <ExerciseBlock key={ex.id} id={ex.id} prompt={ex.prompt} starterCode={ex.starter_code} />)}</div>}
          ...
          <div className="mt-6 flex items-center justify-between gap-3">
            ...
            {isCheck && isLastQuestion ? (
              <button type="button" onClick={completeLesson} disabled={!questionAnswered || saving || lesson.completed} className="inline-flex items-center gap-2 rounded-full bg-mango px-5 py-2.5 text-sm font-semibold text-ink shadow-sm transition-transform hover:scale-[1.02] disabled:opacity-50"><CheckCircle2 size={17} />{lesson.completed ? 'Lesson completed' : saving ? 'Saving…' : 'Finish lesson'}</button>
            ) : (
              <motion.button type="button" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={next} disabled={isCheck && !questionAnswered} className="inline-flex items-center gap-2 rounded-full bg-mango px-5 py-2.5 text-sm font-semibold text-ink shadow-sm disabled:opacity-50">Continue <ArrowRight size={16} /></motion.button>
            )}
          </div>
        </div>
      )}
