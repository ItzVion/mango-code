...
function GoogleSignIn({ onSuccess, onError, disabled }) {
  ...
  useEffect(() => {
    ...
    const load = () => {
      ...
      window.google.accounts.id.renderButton(ref.current, {
        ...
        text: 'continue_with',
        shape: 'pill',
        width: Math.min(ref.current.clientWidth || 360, 400),
      })
      setReady(true)
    }
  ...
  return (
    <div className={`mt-5 ${disabled || !ready ? 'opacity-60' : ''}`} aria-busy={!ready}>
    ...
    <div className="mx-auto max-w-lg py-14">
      <motion.div initial={{ opacity: 0, y: 18, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="relative overflow-hidden rounded-[2rem] border border-ink/10 bg-card p-7 shadow-2xl shadow-ink/10">
        ...
        <div className="relative">
          ...
          <GoogleSignIn disabled={loading} onSuccess={googleLogin} onError={setError} />
